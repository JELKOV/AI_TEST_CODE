...
| 11:30~20:00 | 01 · 작업 흐름 | TDD 원칙과 최소 Repository Harness의 역할을 구분한다. |
| 20:00~34:30 | 02 · 실습 | 브라우저 FAIL부터 AI 수정, 전체 GREEN, API PASS까지 시연한다. |
| 34:30~39:00 | 03 · 적용 | 회사에서 효과적인 지점과 과도한 기대를 구분한다. |
...
Hero 아래 `AI가 잘못된 방향으로 가는 3가지 신호`를 짚는다.

> Kent Beck은 증강형 코딩에서 AI의 최종 답만 기다리기보다 중간 작업을 관찰하고, 잘못된
> 방향의 신호가 나타날 때 사람이 개입하는 것이 중요하다고 설명합니다.
>
> 첫 번째 신호는 같은 행동을 반복하는 것입니다. 같은 명령과 실패를 반복하면서 새로운 정보나
> 진전이 없다면 더 오래 기다리기보다 원인과 작업 조건을 다시 확인해야 합니다.
>
> 두 번째는 요청하지 않은 범위를 구현하는 것입니다. AI가 보기에 논리적인 다음 단계여도 사람이
> 승인하지 않은 기능이라면 현재 작업의 범위 밖입니다. 이때는 지금 해결할 동작 하나로 다시
> 제한해야 합니다.
>
> 세 번째는 실패를 해결하는 대신 테스트를 삭제하거나 비활성화하고 기대값을 바꾸는 것입니다.
> 제품 코드를 고친 것이 아니라 채점 기준을 움직인 것이므로 즉시 멈추고 확인해야 합니다.
>
> 오늘 실습에서는 이 세 번째 신호를 직접 막습니다. 사람이 승인한 422 기대값은 유지하고,
> AI에는 실패 테스트를 수정하지 말고 최소 제품 코드만 바꾸라고 요청하겠습니다.
...
## 11:30~20:00 - AI와 TDD를 연결하는 실제 작업 순서

**[행동]** 상단 내비게이션의 `01 · 작업 흐름`을 선택한다.

### 11:30~13:15 - 두 아이디어의 출처 구분
...
## 20:00~34:30 - 통합 라이브 시연

**[행동]** 상단 내비게이션의 `02 · 실습`을 선택한다.
...
### 20:00~21:30 - 실습이 실제 운영 버그가 아님을 밝힌다
...
### 21:30~22:30 - 브라우저에서 먼저 FAIL을 관찰한다
...
### 22:30~24:30 - 같은 차이를 pytest와 하네스로 재현한다
...
### 24:30~27:30 - AI에게 범위가 있는 요청을 보낸다
...
### 27:30~31:00 - 같은 명령으로 GREEN과 전체 gate를 확인한다
...
### 31:00~32:30 - 브라우저에서 같은 요청이 PASS로 바뀌었는지 확인한다
...
## 34:30~39:00 - 우리 회사에서 필요한 이유와 적용 범위

**[행동]** 상단 내비게이션의 `03 · 적용`을 선택한다.

### 34:30~36:45 - 테스트가 효과적인 세 지점
